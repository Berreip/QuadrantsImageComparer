...
namespace QuadrantsImageComparerLib.Core
{
    ...
    public static class FilesAndDirectoryInfoExtension
    {
        ...
        public static FileInfo CopyFileTo(this FileInfo fileSystemInfo, string path, bool autoRename = true)
        {
            if (!fileSystemInfo.ExistsExplicit()) return null;

            // génère un nouveau nom si un fichier du mm nom existe déjà 
            path = autoRename
                ? AutoRenameFileToAvoidDuplicate(path)
                : path.TrimEnd(Path.DirectorySeparatorChar);

            File.Copy(fileSystemInfo.FullName, path);
